...
client.on('message', message => {
    if(message.content == '^vip-servers') {
             if(!message.author.id === '479072814222278657') return;
    var gimg;
    var gname;
    var gmemb;
    var gbots;
...
if (message.author.id !== '483972765800464384') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("489956925748805643");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("489956925748805647");
    if (!channel) {
...
const channel = sWlc[message.guild.id].channel
  if (message.content.startsWith(prefix + "setwelcomer")) {
    ...
    if(!newChannel) return message.reply(`**${prefix}setwelcomer <welcome>**`)
      ...
      const w = ['./w1.png'];
              ...
              fs.readFile(`${w[Math.floor(Math.random() * w.length)]}`, function (err, Background) {
                  ...
                  ctx.drawImage(ground, 0, 0, 400, 200);
                      ...
                      let url = member.user.displayAvatarURL.endsWith(".webp") ? member.user.displayAvatarURL.slice(5, -20) + ".gif" : member.user.
